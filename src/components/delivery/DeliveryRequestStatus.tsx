...
import { Loader2, CheckCircle, XCircle, Clock, User, Phone, Bike, Bell } from 'lucide-react';
  ...
  const handleBroadcastToPartners = async () => {
    try {
      ...
      if (!vendor?.latitude || !vendor?.longitude) {
        toast({
          title: "Error",
          description: "Vendor location not set. Please update your pharmacy location.",
          variant: "destructive",
        });
        return;
      }
      ...
      if (result.success && result.requestIds && result.requestIds.length > 0) {
        toast({
          title: "Searching for Delivery Partners",
          description: `Notified ${result.requestIds.length} nearby delivery partners`,
        });
        loadRequests();
      } else {
        toast({
          title: "No Partners Found",
          description: result.error || "No delivery partners available in your area right now.",
          variant: "destructive",
        });
      }
    } catch (error) {
      ...
      toast({
        title: "Error",
        description: "Failed to find delivery partners. Please try again.",
        variant: "destructive",
      });
    } finally {
  ...
  if (requests.every((r: any) => r.status !== 'accepted')) {
    return (
      <Card className="border-red-500 bg-red-50/50 dark:bg-red-950/20">
        ...
        <CardContent className="space-y-3">
          <p className="text-sm text-muted-foreground">
            No delivery partners accepted the request. Try broadcasting again to find available partners.
          </p>
          <div className="flex gap-2">
            ...
            >
              ...
              {broadcasting ? 'Searching...' : 'Find Partners'}
            </Button>
          </div>
        </CardContent>
      </Card>
